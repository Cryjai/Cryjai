<div align="center">

# Acry Chak / Cryjai

### Hong Kong builder · self-taught product hacker · AI-native learner

I build weirdly useful apps at the intersection of **productivity, education, finance, and Chinese metaphysics**.  
Not here to collect tutorials. Here to ship things.

[Portfolio](https://cryjai.github.io/Acry) · [GitHub Projects](https://github.com/Cryjai?tab=repositories) · [Destiny Web App](https://cryjai.github.io/Destiny/) · [Routine Monsters](https://github.com/Cryjai/RoutineMonsterApp) · [易經起卦](https://cryjai.github.io/ichingpredict/)

</div>

---

## About

- Born in 2007, based in Hong Kong.
- Self-study student with ADHD / SEN traits, fast-learning, high-obsession mode, strong builder energy.
- Interested in **product design (Functions and UI), behavioral systems, finance, language learning, and strategic thinking**.
- Goal: use free resources to build real apps, compound skills fast, and become financially independent through products.

---

## What I’m building

### Featured Projects

| Project | What it is | Stack |
|---|---|---|
| [Destiny](https://github.com/Cryjai/Destiny) | A Chinese metaphysics app with a modern UI, covering BaZi, Zi Wei Dou Shu, I Ching, and daily fortune flows | React, Vite, Tailwind |
| [Routine Monsters](https://github.com/Cryjai/RoutineMonsterApp) | A gamified offline-first habit tracker where routines become monsters you raise or neglect | React, TypeScript, Capacitor |
| [中醫穴位for亞健康指南](https://cryjai.github.io/ChineseAcupressureSite/) | Chinese Acupressure Site for relieving | Web app| 
| [Penguin Purse App](https://github.com/Cryjai/PenguinPurseApp) | Personal Finance, budget ,money tracker| Android app |
| [Acry Finance Hub](https://cryjai.github.io/acry-finance-hub/#) | Personal finance / stock-tracking / money-thinking experiments | Frontend web app |
| [Acry DSE Study Hub](https://github.com/Cryjai/Acry-dse-study-hub) | Curated study resources and self-learning materials | Resource hub |

---

## Current focus

- Shipping more polished mobile/web apps.
- Getting stronger at **TypeScript, frontend architecture, product thinking, and full-stack workflows**.
- Turning personal pain points into usable products.
- Building a portfolio that is useful, sharp, and commercially viable.

---

## Style of work

- Learn fast.
- Build in public.
- Use constraints as leverage.
- Prefer free tools, fast iteration, and actual output over fake perfection.

---

## Interests

- Product building
- Behavioral design
- Quant / finance thinking
- Chinese philosophy and strategy
- Spanish learning
- AI-assisted workflows
- Education tools that are actually useful

---

## Selected links

- [Portfolio site](https://cryjai.github.io/Acry)
- [Destiny live demo](https://cryjai.github.io/Destiny/)
- [Routine Monsters repo](https://github.com/Cryjai/RoutineMonsterApp)
- [All repositories](https://github.com/Cryjai?tab=repositories)

---

<div align="center">

**Building products before permission.**

</div>
